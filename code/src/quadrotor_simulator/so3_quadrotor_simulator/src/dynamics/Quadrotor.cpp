                      ...
                      Quadrotor::InternalState& dxdt, const double /* t */)
{
  ...
  // v_dot = //?????
  v_dot = -g_ * Eigen::Vector3d::UnitZ()  + (thrust / mass_) * R.col(2)     + external_force_ / mass_ 
        - (resistance / mass_) * vnorm;
  ...
  // omega_dot = //??????
  omega_dot = J_.inverse() * (moments                   // 螺旋桨产生的力矩
            - cur_state.omega.cross(J_ * cur_state.omega) // 科里奥利力项
